...
SESSION_AUDIO_DIR = "storage/temp_audio/sessions"

SELF_ANALYSIS_AUDIO_DIR = (
    "storage/temp_audio/self_analysis"
)
...
async def upload_self_analysis_audio(
    psychologist_id: str,
    session_id: str,
    file: UploadFile
):
    ...
    secure_filename = (
        f"self_analysis_{session_id}_{uuid.uuid4()}{extension}"
    )
    ...
    (
        ...
        .table("sessions")
        .update({
            "self_analysis_audio_url": file_path,
            "self_analysis_stt_status": "recorded"
        })
        .eq("id", session_id)
        .eq("psychologist_id", psychologist_id)
        .execute()
